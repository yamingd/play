...
class SolrCoreAdmin(object):
    ...
    """
    def __init__(self,url=settings.search.url,solr_class=Solr):
        self.url = url
    ...
    def create_core(self,name):
        ...
        create_cmd = self.core_admin_url + urllib.urlencode({'action':'create','name':name,'loadOnStart':'true', 'instanceDir':'.','dataDir':settings.search.data + '/' + name})
